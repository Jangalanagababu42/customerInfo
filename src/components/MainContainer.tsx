...
const MainContainer: React.FC<props> = ({ currCustomer, images }) => {
  return (
    <div className="mx-10 ">
      ...
      <p className="text-center mt-5 w-10/12 m-auto">
        {currCustomer % 2 === 0
        ...
        {images.map((image, index) => (
          <div key={index}>
            <img
              src={image}
              alt={`Imag ${index}`}
              className="rounded-lg shadow-md w-7/12 m-auto h-[120px] p-4 hover:scale-105 transition-transform duration-1000 ease-in-out"
            />
          </div>
        ))}
      </div>
